/*global define*/
// base-source https://github.com/kevindente/expect-dom
define(['jquery', 'test/lib/expect'], function (jQuery, expect) {
	...
	var assertions = {
		...
		attr: function(attrName, expectedValue) {
			if (expectedValue) {
				this.assert(
					this.obj.attr(attrName) == expectedValue,
		...
		html: function(html) {
			this.assert(
				this.obj.html() === normalizeHtmlTagCase(html),
		...
		data: function(key, expectedValue) {
			if (expectedValue) {
				this.assert(
					this.obj.data(key) == expectedValue,
		...
		handle: function(eventName) {
			...
			this.assert(
				events && events[eventName] && events[eventName].length > 0,
